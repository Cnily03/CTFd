...
from .extensions import get_mode, log
...
def load(app):
    ...
    app.db.create_all()
    if "owl_setup" not in DBUtils.get_all_configs():
    ...
    CHALLENGE_CLASSES["dynamic_docker_owl"] = DynamicCheckValueChallenge
    ...
    def admin_delete_container():
        user_id = request.args.get("user_id")
        challenge_id = request.args.get("challenge_id")
        ControlUtil.destroy_container(user_id, challenge_id)
        return jsonify({"success": True})
        ...
        try:
            ...
            if data is not None:
                ...
                dynamic_docker_challenge = DynamicCheckChallenge.query.filter(
                    DynamicCheckChallenge.id == data.challenge_id
                ).first_or_404()
                            ...
                            return jsonify(
                                {
                                    "success": True,
                                    "type": "http",
                                    "domain": data.docker_id + "." + domain,
                                    "remaining_time": timeout
                                    - (
                                        datetime.datetime.utcnow() - data.start_time
                                    ).seconds,
                                    "lan_domain": lan_domain,
                                }
                            )
        ...
        try:
            ...
            if ControlUtil.frequency_limit():
                return jsonify(
                    {
                        "success": False,
                        "msg": "Frequency limit, You should wait at least 1 min.",
                    }
                )
            # check whether the user has boot too many containers
            MAX_USER_OPEN_CONTAINERS = int(configs.get("docker_max_opened_count"))
            openedContainers = DBUtils.get_current_containers(user_id)
            if openedContainers and len(openedContainers) >= MAX_USER_OPEN_CONTAINERS:
                return jsonify(
                    {
                        "success": False,
                        "msg": "Up to {} container{} can be opened at the same time.".format(
                            MAX_USER_OPEN_CONTAINERS,
                            "s" if MAX_USER_OPEN_CONTAINERS > 1 else "",
                        ),
                    }
                )
            else:
                ...
                if configs.get("docker_max_container_count") != "None":
                    if int(configs.get("docker_max_container_count")) <= int(
                        ...
                        return jsonify(
                            {"success": False, "msg": "Max container count exceed."}
                        )

                dynamic_docker_challenge = DynamicCheckChallenge.query.filter(
                    DynamicCheckChallenge.id == challenge_id
                ).first_or_404()
                try:
    ...
    def destroy_container():
        ...
        if ControlUtil.destroy_container(user_id, challenge_id):
            return jsonify({"success": True})
        else:
            return jsonify(
                {
                    "success": False,
                    "msg": "Failed when destroy instance, please contact with the admin!",
                }
            )
    ...
    def renew_container():
        ...
        container = ControlUtil.get_container(user_id, int(challenge_id))
        ...
        ControlUtil.expired_container(user_id=user_id, challenge_id=int(challenge_id))
    ...
    def auto_clean_container():
        with app.app_context():
            ...
            for r in results:
                ControlUtil.destroy_container(r.user_id, r.challenge_id)
